...
public class Command_kickall extends CommonCommand {
	...
	public boolean run(CommandSender sender, Command cmd, String[] args) {
		...
		if (args.length == 0) {
			...
			MessageManager.log(Level.FATAL, "The server has been kicked by " + player.getName());
			return true;
		}
		if (args.length == 1) {
			String msg = "";
			...
			player.sendMessage(Red + "Kicked all players from the server.");
			MessageManager.log(Level.FATAL, "The server has been kicked by " + player.getName() + " for " + msg);
			return true;
		}
		return true;
	}
