...
public class BarApiUtil {
	...
	public static Class<?> fakeDragonClass = BarApiv1_6.class;
	...
	static {
		...
		if (versions[0].equals("v1") & Integer.parseInt(versions[1]) > 6) {
			newProtocol = true;
			fakeDragonClass = BarApiv1_7.class;
		}

		version = mcVersion + ".";
	}

	public static BarApiFakeDragon newDragon(String message, Location loc) {
		BarApiFakeDragon fakeDragon = null;

		try {
			fakeDragon = (BarApiFakeDragon) fakeDragonClass.getConstructor(String.class, Location.class).newInstance(message, loc);
