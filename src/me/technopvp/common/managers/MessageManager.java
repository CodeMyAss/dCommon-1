...
public class MessageManager extends CommonCore {
	...
	public static void message(CommandSender sender, String message, boolean colorize) {
		sender.sendMessage((colorize == true ? StringUtils.colorize(message)
				: message));
	}
	...
	public static void broadcast(BroadcastType castType, String message, boolean colorize) {
		broadcast(castType, (colorize == true ? StringUtils.colorize(message) : message));
	}
	 ...
	 *            - The string, that will be sent.
	 *
	 * @see broadcastWPermissionLevel#broadcast(castType, permissionLevel, message)
	 */
